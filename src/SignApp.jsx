...
const seedTransactions = () => {
  ...
  return [
    { id: uid(), type: "expense", category: "Groceries", amount: 640, date: mk(1), note: "Big Bazaar" },
    { id: uid(), type: "expense", category: "Food & Drinks", amount: 380, date: mk(1), note: "Cafe with friends" },
    { id: uid(), type: "expense", category: "Transport", amount: 120, date: mk(2), note: "Auto" },
    { id: uid(), type: "income", category: "Allowance", amount: 5000, date: mk(3), note: "Monthly allowance" },
    { id: uid(), type: "expense", category: "Shopping", amount: 1450, date: mk(4), note: "New sneakers" },
    { id: uid(), type: "expense", category: "Entertainment", amount: 300, date: mk(5), note: "Movie night" },
  ];
};
...
function parseStatementText(text) {
  // Looks for lines with a date + amount + optional Dr/Cr or +/-
  const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
  const dateRe = /(\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{2,4})/;
  const amtRe = /(-?₹?\s?[\d,]+\.\d{2}|-?₹?\s?[\d,]+)/g;
  const results = [];
  ...
  for (const line of lines) {
    ...
    // take the last numeric token as amount (common in statement rows)
    let raw = amounts[amounts.length - 1].replace(/[₹,\s]/g, "");
    let amt = parseFloat(raw);
    if (isNaN(amt) || amt === 0) continue;

    const isCredit = /cr\b|credit/i.test(line) || amt > 0 && /\+/.test(line);
    const isDebit = /dr\b|debit/i.test(line);
    ...
    if (isNaN(new Date(iso).getTime())) continue;

    const desc = line.replace(dateRe, "").replace(amtRe, "").trim().slice(0, 60) || "Statement entry";

    results.push({
      id: uid(),
      type: isDebit ? "expense" : isCredit ? "income" : (amt < 0 ? "expense" : "income"),
      category: isDebit || amt < 0 ? "Other" : "Other",
      amount: Math.abs(amt),
      date: iso,
      note: desc,
      fromStatement: true,
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center sm:justify-center">
      ...
      <div className="relative w-full sm:max-w-md bg-[#F7F4EC] rounded-t-2xl sm:rounded-sm border-2 border-[#0F1410] p-5 pb-8 sm:pb-5 safe-bottom max-h-[85vh] overflow-y-auto">
        ...
        <p className="text-xs text-[#7A8B7F] mb-3 leading-relaxed">
          Open your bank statement PDF, select all the transaction text, and paste it below.
          I'll find dates and amounts and turn them into ledger entries — review before adding.
        </p>
        <textarea
          value={text}
        ...
        {!preview ? (
          <button
            onClick={runParse}
            disabled={!text.trim()}
            className="mt-3 w-full bg-[#0F1410] disabled:opacity-30 text-[#F7F4EC] font-semibold py-2.5 rounded-sm"
          >
            Find transactions
          </button>
        ) : (
          <div className="mt-3">
            {preview.length === 0 ? (
              <AdvisorNote tone="warn">
                Couldn't find recognizable transaction lines. Try pasting rows that include a date and an amount together.
              </AdvisorNote>
            ) : (
              <>
                <p className="text-xs font-semibold text-[#0F1410] mb-2">
                  Found {preview.length} entries — {preview.filter(p=>p.type==='income').length} income, {preview.filter(p=>p.type==='expense').length} expense
                </p>
                <div className="max-h-48 overflow-y-auto border-2 border-[#0F1410]/10 rounded-sm divide-y divide-[#0F1410]/10 mb-3">
                  {preview.map((p) => (
                    <div key={p.id} className="flex justify-between items-center px-3 py-2 text-xs">
                      <div className="min-w-0">
                        ...
                        <div className="text-[#7A8B7F]">{p.date}</div>
                      </div>
                ...
                >
                  Add all {preview.length} to ledger
                </button>
              </>
            )}
          </div>
...
export default function SignApp() {
  ...
  const startApp = (bal) => {
    ...
    setGoals([
      { id: uid(), name: "Dandiya event", target: 2000, saved: 400, deadline: null },
      { id: uid(), name: "New phone", target: 25000, saved: 3000, deadline: null },
    ]);
    setOnboarded(true);
  };
