...
import {
  ...
  Cloud, CloudOff, Loader2, Send, CheckCircle2, HelpCircle,
} from "lucide-react";
...
function AddTransactionSheet({ type, onClose, onAdd, goals }) {
  const [category, setCategory] = useState(
    type === "expense" ? EXPENSE_PRESETS[0].name : type === "income" ? INCOME_PRESETS[0] : "Savings"
  );
  ...
  const submit = () => {
    ...
    onAdd({
      id: uid(),
      type,
      category: type === "saving" ? "Savings" : (useCustom && customCat.trim() ? customCat.trim() : category),
      amount: amt,
      date,
      note: type === "saving" ? (note.trim() || `Added to ${selectedGoal ? selectedGoal.name : "goal"}`) : note.trim(),
...
function UpiPaySheet({ onClose, onAdd }) {
  const [stage, setStage] = useState("form"); // 'form' | 'awaiting' | 'confirm'
  const [upiId, setUpiId] = useState("");
  const [name, setName] = useState("");
  const [amount, setAmount] = useState("");
  const [note, setNote] = useState("");
  const [category, setCategory] = useState(EXPENSE_PRESETS[0].name);
  const [useCustom, setUseCustom] = useState(false);
  const [customCat, setCustomCat] = useState("");
  const [error, setError] = useState(null);

  const upiIdValid = /^[\w.\-]{2,}@[\w.\-]{2,}$/.test(upiId.trim());
  const canPay = upiIdValid && parseFloat(amount) > 0;

  // detect when the person comes back to this tab after the UPI app opened
  useEffect(() => {
    ...
    // also catch the case where the browser regains focus without a visibility change firing
  ...
  const openUpiApp = () => {
    ...
    setStage("awaiting");
    // small delay so the "awaiting" UI paints before we hand off to the UPI app
  ...
  const confirmPaid = () => {
    onAdd({
      ...
      date: todayISO(),
      note: note.trim() || (name.trim() ? `Paid to ${name.trim()}` : "UPI payment"),
    });
    onClose();
  };
      ...
      <div className="relative w-full sm:max-w-sm bg-[#F7F4EC] rounded-t-2xl sm:rounded-sm border-2 border-[#0F1410] p-5 pb-8 sm:pb-5 safe-bottom max-h-[85vh] overflow-y-auto">
        ...
        {stage === "form" && (
          <>
            ...
            <p className="text-xs text-[#7A8B7F] mb-4 leading-relaxed">
              This opens your UPI app (GPay, PhonePe, etc.) with these details filled in. You pay there, using your own bank login — $ign never sees your bank details or moves money itself.
            </p>
            ...
            <div className="mt-3">
              <Field label="Recipient's name (optional)">
                <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Who are you paying?" className={inputCls} />
              </Field>
            </div>
              ...
              <Field label="Category">
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {EXPENSE_PRESETS.map((p) => (
                    <button
                      key={p.name}
                      onClick={() => { setCategory(p.name); setUseCustom(false); }}
                      className={`px-2.5 py-1.5 rounded-full text-xs font-medium border-2 transition-colors ${
                        !useCustom && category === p.name
                          ? "bg-[#0F1410] text-[#F7F4EC] border-[#0F1410]"
                          : "bg-white text-[#0F1410] border-[#0F1410]/15"
                      }`}
                    >
                ...
                {useCustom && (
                  <input value={customCat} onChange={(e) => setCustomCat(e.target.value)} placeholder="Name your category" className={inputCls} />
                )}
              </Field>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-3">
              ...
              <Field label="Note (optional)">
                <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="What's this for?" className={inputCls} />
              </Field>
            </div>

            {upiId.trim() && !upiIdValid && (
              <p className="text-[11px] text-[#C0483A] mt-2">That doesn't look like a valid UPI ID — it should look like name@bank.</p>
            )}
            {error && <div className="mt-3"><AdvisorNote tone="warn">{error}</AdvisorNote></div>}

            <button
              onClick={openUpiApp}
              disabled={!canPay}
              className="mt-5 w-full flex items-center justify-center gap-2 bg-[#2E6B4F] disabled:opacity-30 text-[#F7F4EC] font-semibold py-3 rounded-sm"
            >
              <Send size={16} /> Open UPI app to pay
            </button>
          </>
        )}
        ...
        {stage === "awaiting" && (
          <div className="py-10 flex flex-col items-center text-center gap-3">
            <Loader2 size={28} className="animate-spin text-[#2E6B4F]" />
            <p className="text-sm text-[#0F1410] font-medium">Opening your UPI app…</p>
            <p className="text-xs text-[#7A8B7F]">Come back here once you've paid (or cancelled).</p>
            <button onClick={() => setStage("confirm")} className="text-xs underline text-[#7A8B7F] mt-2">
              I'm back
            </button>
          </div>
        )}
        ...
        {stage === "confirm" && (
          <div className="py-6">
            <div className="flex flex-col items-center text-center gap-2 mb-6">
              <HelpCircle size={28} className="text-[#B8935A]" />
              <p className="text-base font-bold text-[#0F1410]">
                Did the payment of {fmt(parseFloat(amount) || 0)}{name.trim() ? ` to ${name.trim()}` : ""} go through?
              </p>
              <p className="text-xs text-[#7A8B7F]">$ign can't check this automatically — let me know so your ledger stays accurate.</p>
            </div>
            <div className="flex gap-3">
              <button
                onClick={notPaid}
                className="flex-1 border-2 border-[#0F1410] text-[#0F1410] font-semibold py-3 rounded-sm text-sm"
              >
                No / cancelled
              </button>
              <button
                onClick={confirmPaid}
                className="flex-1 flex items-center justify-center gap-1.5 bg-[#2E6B4F] text-[#F7F4EC] font-semibold py-3 rounded-sm text-sm"
              >
                <CheckCircle2 size={16} /> Yes, log it
              </button>
            </div>
          </div>
        )}
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <div className="flex-1 min-w-0 bg-white border-2 border-[#0F1410]/10 rounded-sm p-3">
          ...
          <div className="text-base font-bold text-[#2E6B4F] font-mono break-all">{fmt(rt.inc)}</div>
        </div>
        <div className="flex-1 min-w-0 bg-white border-2 border-[#0F1410]/10 rounded-sm p-3">
          <div className="text-[10px] uppercase tracking-wide text-[#7A8B7F] font-semibold mb-1 flex items-center gap-1">
            <TrendingDown size={12} /> out
          </div>
          <div className="text-base font-bold text-[#C0483A] font-mono break-all">{fmt(rt.exp)}</div>
        </div>
        <div className="flex-1 min-w-0 bg-white border-2 border-[#0F1410]/10 rounded-sm p-3">
          <div className="text-[10px] uppercase tracking-wide text-[#7A8B7F] font-semibold mb-1">net</div>
          <div className={`text-base font-bold font-mono break-all ${rt.inc - rt.exp >= 0 ? "text-[#0F1410]" : "text-[#C0483A]"}`}>
...
export default function SignApp() {
  ...
  const [sheet, setSheet] = useState(null); // 'expense' | 'income' | 'saving' | 'goal' | 'statement' | 'settings' | 'upi'
  ...
  return (
    <div className="min-h-screen bg-[#EDE9DD] pb-32 overflow-x-hidden" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      {/* header */}
        ...
        {tab === "home" && (
          <>
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              <button onClick={() => setSheet("upi")} className="col-span-2 bg-[#0F1410] text-[#F7F4EC] rounded-sm py-3 flex items-center justify-center gap-2 text-sm font-semibold" style={{boxShadow:"3px 3px 0 #B8935A"}}>
              ...
              <button onClick={() => setSheet("statement")} className="col-span-2 bg-[#B8935A] text-[#0F1410] rounded-sm py-3 flex items-center justify-center gap-2 text-xs font-semibold" style={{boxShadow:"3px 3px 0 #0F1410"}}>
      ...
      {sheet === "upi" && <UpiPaySheet onClose={() => setSheet(null)} onAdd={addTransaction} />}
      ...
      {sheet === "settings" && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center sm:justify-center">
          ...
          <div className="relative w-full sm:max-w-sm bg-[#F7F4EC] rounded-t-2xl sm:rounded-sm border-2 border-[#0F1410] p-5 pb-8 sm:pb-5 safe-bottom max-h-[85vh] overflow-y-auto">
            ...
            <div className="mt-5 pt-4 border-t-2 border-[#0F1410]/10">
              {!confirmReset ? (
                <button
                  onClick={() => setConfirmReset(true)}
                  className="w-full flex items-center justify-center gap-2 text-[#C0483A] font-semibold py-2.5 text-sm"
                >
                  <Trash2 size={14} /> Reset ledger
                </button>
              ) : (
                <div className="p-3 bg-[#C0483A]/10 border-2 border-[#C0483A]/30 rounded-sm">
                  <p className="text-[11px] text-[#0F1410] mb-3 leading-relaxed">
                    This deletes all transactions, goals, and your saved balance from this browser — permanently. Consider downloading a backup first. Are you sure?
                  </p>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setConfirmReset(false)}
                      className="flex-1 border-2 border-[#0F1410] text-[#0F1410] font-semibold py-2 rounded-sm text-xs"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={() => { resetLedger(); setConfirmReset(false); setSheet(null); }}
                      className="flex-1 bg-[#C0483A] text-[#F7F4EC] font-semibold py-2 rounded-sm text-xs"
                    >
